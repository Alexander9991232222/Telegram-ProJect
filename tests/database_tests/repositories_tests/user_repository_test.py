...
@pytest.fixture
def persisted_user(temp_db, user_data):
...
def test_user_repository_create(temp_db, user_data):
    with temp_db.get_session() as session:
        repo = UserRepository(session)
...
def test_user_repository_update(temp_db, persisted_user):
    with temp_db.get_session() as session:
        repo = UserRepository(session)
...
def test_user_repository_delete(temp_db, persisted_user):
    with temp_db.get_session() as session:
        repo = UserRepository(session)
...
def test_user_repository_get_by_filters(temp_db):
    with temp_db.get_session() as session:
        repo = UserRepository(session)
