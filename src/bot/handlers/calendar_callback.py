...
@calendar_router.callback_query(F.data.startswith("mcal_date:"))
...
@calendar_router.callback_query(F.data == "mcal_confirm")
async def confirm_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"Ви обрали {len(selected)} дат: \n" + "\n".join(selected))
    await state.clear()